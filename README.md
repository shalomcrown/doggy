...
- On a **rover**, `/` is `rover.html`: steering and speed sliders (0 centered, `[-1, 1]`) send `POST /api/drive` after 100 ms idle. Speed magnitude maps linearly to 0–4095 PWM ticks on all enabled motors; its sign selects direction. Steering is stored and reported but is not mixed into motor outputs yet. The table shows each motor’s commanded PWM, enable, and configured wiring direction.
