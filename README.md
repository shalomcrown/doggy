...
Logs go to `/var/log/doggy/doggy.log` (override with `DOGGY_LOG_DIR`). The file is rolled on every start and when it exceeds the size cap; 10 files are kept and rolled copies are named `doggy-YYYY-MM-DD-HHMM.log.gz`. The log directory is `0755` and log files are world-readable (`0644`). Every `/api/...` request is logged with the **remote IP** first (`127.0.0.1` is `doggy-lora` polling `GET /api/config` every 5 s in robot mode, not the web UI). Endpoints with a request body log one-line JSON (PIN and air key redacted on config/system calls); `GET /api/config` logs one-line public JSON from the response. HTTP failures (4xx/5xx) and hardware errors are logged at error severity. The packaged unit sets `LogsDirectory=doggy` so the directory is owned by user `doggy`.
